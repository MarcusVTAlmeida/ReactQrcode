...
const handleImageUpload = async (e) => {
  ...
  if (file && (file.type === 'image/png' || file.type === 'image/svg+xml')) {
    ...
    reader.onload = (event) => {
      const base64Image = event.target.result; // Imagem convertida para base64
  ...
  const handleDownloadQRCode = () => {
    const canvas = qrCodeRef.current.querySelector('canvas'); // Seleciona o canvas gerado pelo QRCodeCanvas
    if (!canvas) {
